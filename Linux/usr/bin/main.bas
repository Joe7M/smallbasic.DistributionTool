...
sub LINUX_PackageVersion_WindowsSelfExtracting()
    ...
    'dest = enclose(DestinationDirectory.value + "/Win_SelfExtracting/PackagingTemp.7z")
    ...
    'source = enclose(DestinationDirectory.value + "/Win_SelfExtracting/PackagingTemp.7z")
    'dest = enclose(DestinationDirectory.value + "/Win_SelfExtracting/Setup.exe")

    'PrintStatus("Self Extracting Archive: Create self extracting archive", 50)

    's = "7z a -mx7 -sfx\"./data/win64/7z.sfx\" " + dest + " " + source
    'v = run(s)
